...
function renderItems() {
    ...
    const categoryRecipes = recipes.filter(recipe => recipe.category === selectedCategory);

    if (categoryRecipes.length === 0) {
        itemsList.innerHTML = '<div class="no-items">No items in this category</div>';
        return;
    }
    ...
    categoryRecipes.forEach(recipe => {
        ...
        for (const ingredient of recipe.ingredients) {
            const playerHas = inventory[ingredient.item] ? inventory[ingredient.item].amount : 0;
...
function updateSkillDisplay() {
    ...
    document.getElementById('skill-progress').style.width = `${Math.min(skillLevel, 100)}%`;
}
